...
class CrossAttention(nn.Module):
    ...
    def __init__(self, dim, rope=None, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., softmax_camap=False):   ###
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = head_dim ** -0.5
    ...
    def forward(self, query, key, value, qpos, kpos):
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale   # b 12 196 196 = b num_heads num_tokens num_tokens
        attn_tmp = attn.clone().detach()  
        attn = attn.softmax(dim=-1)

        if self.softmax_camap:  # return softmax applied attention map
            # print('softmax applied camap')
            attn_tmp = attn.clone().detach()
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, rope=rope, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        self.cross_attn = CrossAttention(dim, rope=rope, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, softmax_camap=False)   #####
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
