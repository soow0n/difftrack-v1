...
def sample_queries_first(
    ...
    for i in range(target_points.shape[0]):
        ...
        query_points.append(np.array([index, x, y]))  # [t, y, x]
...
class TAPVid(torch.utils.data.Dataset):
    def __init__(self, args, visualize=False):
        ...
        if "kinetics" in self.dataset_type:
            ...
            if args.end == 0:
                self.points_dataset = points_dataset[args.start:]
            else:
                self.points_dataset = points_dataset[args.start:args.end]
    ...
    def __getitem__(self, index):
        ...
        if self.resize_shape is not None:
            ...
            frames = resize_video(frames, [self.resize_shape[0], self.resize_shape[1]])
        ...
        if self.visualize:
            rgbs_ori = torch.from_numpy(frames_ori).permute(0, 3, 1, 2).float()
            return rgbs, trajs, visibles, query_points, rgbs_ori
