...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> inversion_pipe = CogVideoXInversePipeline.from_pretrained(model_path, torch_dtype=dtype)
        >>> inversion_pipe.scheduler = CogVideoXDDIMScheduler.from_config(inversion_pipe.scheduler.config, timestep_spacing="trailing")
        >>> inversion_pipe.to(device="cuda", dtype=dtype)
...
class CogVideoXInversePipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
    ...
    def __call__(
        ...
        timesteps = torch.concat([torch.tensor([0]).to(device=device), timesteps])
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if i > num_inference_steps - inverse_step - 1:
                    break
                if self.interrupt:
                    continue
                ...
                noise_pred = output.sample.float()
                ...
                latents = self.scheduler.inverse_step(noise_pred, t, latents, return_dict=False)[0]
