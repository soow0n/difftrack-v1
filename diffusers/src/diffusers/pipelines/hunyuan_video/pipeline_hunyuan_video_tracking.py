...
class HunyuanVideoTrackPipeline(DiffusionPipeline, HunyuanVideoLoraLoaderMixin):
    r"""
    ...
    def prepare_latents(
        ...
        latents: Optional[torch.Tensor] = None, # [B, C, F, H, W]
        video: list = None, # [B, F, C, H, W]
        ...
        if frame_as_latent:
            ...
            latents = torch.cat(init_latents, dim=0).to(dtype).permute(2, 0, 1, 3, 4)  # [B, C, F, H, W]
        else:
            num_frames = (video.size(2) - 1) // self.vae_scale_factor_temporal + 1 if latents is None else latents.size(2)
            if isinstance(generator, list):
                init_latents = [
            ...
            latents = torch.cat(init_latents, dim=0).to(dtype) # [B, C, F, H, W]
