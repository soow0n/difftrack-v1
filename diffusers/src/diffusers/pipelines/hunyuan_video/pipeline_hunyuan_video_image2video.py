...
class HunyuanVideoImageToVideoPipeline(DiffusionPipeline, HunyuanVideoLoraLoaderMixin):
    ...
    def __call__(
        ...
        evaluator = Evaluator(save_dir=os.path.join(output_dir, 'pck'), device=gt_track.device)
        vis_trajectory = None
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #### Trajectory & Attention Score ####
                text_len = prompt_embeds.size(1)
                _, _, f_num, h, w = latents.shape
                h, w = h//2, w//2
                ...
                with torch.no_grad():
                    for l, blk in enumerate(self.transformer.transformer_blocks):
                        ...
                        if params['attn_weight']:
                            ...
                            text_score, _ = attn_weight[:, -text_len:].max(dim=-1)
                            ...
                            for f in range(1, f_num):
                                ...
                                latent_visible_mask = gt_visibility[0][4*f-3:4*f+1].to(cross_score_max.device).all(dim=0).float()
                            ...
                            attn_score_df.loc[(f"timestep{i}", "self"), f"layer{l}"] = round(self_score_grid_mean.item(), 4)
                            attn_score_df.loc[(f"timestep{i}", "cross"), f"layer{l}"] = round(cross_score_grid_mean.item(), 4)
                            attn_score_df.loc[(f"timestep{i}", "text"), f"layer{l}"] = round(text_score_grid_mean.item(), 4)
                        ...
                        if params['trajectory']:
                            ...
                            evaluator.measure(pred_tracks=trajectory, gt_tracks=gt_track, gt_visibility=gt_visibility, mode="default",
                                filename_thres=f"layer_{l}_timestep_{int(t)}.txt",
                                filename_overall=f"layer_{l}.txt",
                                timestep=int(t)
                            )
                            if params['vis_timesteps'][0] == i and params['vis_layers'][0] == l:
        ...
        if not return_dict:
            return (video, attn_score_df, vis_trajectory)
