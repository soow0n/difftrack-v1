...
class CogVideoXDDIMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.init_alpha_cumprod = torch.tensor(1.0) if set_alpha_to_one else self.alphas_cumprod[-1]
    ...
    def inverse_step(
        ...
        # 2. compute alphas, betas
        alpha_prod_t = self.alphas_cumprod[timestep]
        alpha_prod_t_next = self.alphas_cumprod[next_timestep] if next_timestep <= self.config.num_train_timesteps else self.init_alpha_cumprod
