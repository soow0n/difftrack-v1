...
def process_chunk(
    ...
    return query_frames, key_frames, frames
...
def main(args):
    ...
    for j, (video, gt_trajectory, visibility, query_points_i, video_ori) in enumerate(dataloader):
        ...
        while len(visited_idx) < T - 1:
            ...
            # Process the chunk.
            query_frames, key_frames, frames = process_chunk(
            ...
            for chk, chunk_idx in enumerate(chunk_indices):
                # if frame_as_latent:
                background = chunk_video[0][chk].permute(1, 2, 0)
                ...
                attn_map = cv2.addWeighted(background.astype(np.uint8), 0.3, colormapped_im, 0.7, 0)
